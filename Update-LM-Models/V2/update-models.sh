...
# If both failed, show message and exit without changes
if [[ $OLLAMA_SUCCESS -ne 0 && $LMSTUDIO_SUCCESS -ne 0 ]]; then
    echo "Both Servers could not be reached - No Changes were Made"
    exit 0
fi
...
# Show update message
if [[ $OLLAMA_SUCCESS -eq 0 && $LMSTUDIO_SUCCESS -eq 0 ]]; then
    echo "Both Ollama & LM Studio models were Updated!"
elif [[ $OLLAMA_SUCCESS -eq 0 ]]; then
    echo "Only the Ollama models were Updated!"
elif [[ $LMSTUDIO_SUCCESS -eq 0 ]]; then
    echo "Only the LM Studio models were Updated!"
fi
